...
class Player(Object): # player is a subclass of object from game.py
    ...
    def drawSprite(self):
        ...
        if self.direction == 0:
            self.ReadSurface().blit(self.player_images[counter // 5], (self.ReadXPos(), self.ReadYPos())) # cycle the john man frames on a timer
        elif self.direction == 1:
            self.ReadSurface().blit(pygame.transform.flip(self.player_images[counter // 5], True, False,) (self.ReadXPos(), self.ReadYPos())) # the pygame transform flip changes the direction of the sprite and the true/false tells it what axis to flip
